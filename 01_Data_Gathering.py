...
# Airport Locations einlesen
df10 = pd.read_csv('GlobalAirportDatabase_Part4.csv', sep=';')
print (df10)

df10['LAT']=df10['LAT'].astype(float)
df10['LON']=df10['LON'].astype(float)

for i in range(0, len(df10)):

    # Location und Zeitperiode für meteostat Abfrage definieren
    start = datetime(1991, 1, 1)
    end = datetime(2021, 12, 31)
    location_lat=df10['LAT'].iloc[i]
    location_lon=df10['LON'].iloc[i]
